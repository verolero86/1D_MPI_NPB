...
// of the sweep.
//---------------------------------------------------------------------
void y_solve(int rank, int size, int root, int kstart, int kend, int kdelta, int *kdeltas, int *disps)
{
    int i, j, k, m, n, jsize;
...
// #pragma omp parallel for default(shared) shared(jsize) private(i,j,k,m,n)
    ...
    // Send upper boundary from even rank to rankp1
    // This implies odd rank will receive in location kstart - 1
    // scount = (JMAXP+1)*(IMAXP+1)*5;
    // rcount = scount;

    // tright = 0;
    // if(rank == 0) {
    //     MPI_Send(&rhs[kend-1][0][0],scount,MPI_DOUBLE,rankp1,tright,MPI_COMM_WORLD);
    // } else {
    //     MPI_Recv(&rhs[kstart-1][0][0],rcount,MPI_DOUBLE,rankm1,tright,MPI_COMM_WORLD,&status);
    //     if(rankp1 < size) {
    //         MPI_Send(&rhs[kend-1][0][0],scount,MPI_DOUBLE,rankp1,tright,MPI_COMM_WORLD);
    //     }
    // }
    ...
    // tleft = 1;
    // if(rank == size-1) {
    //     MPI_Send(&rhs[kstart][0][0],scount,MPI_DOUBLE,rankm1,tleft,MPI_COMM_WORLD);
    // } else {
    //     MPI_Recv(&rhs[kend][0][0],rcount,MPI_DOUBLE,rankp1,tleft,MPI_COMM_WORLD,&status);
    //     if(rankm1 >= 0) {
    //         MPI_Send(&rhs[kstart][0][0],scount,MPI_DOUBLE,rankm1,tleft,MPI_COMM_WORLD);
    //     }
    // }
    ...
    // rcount = scount;
    // errcode = MPI_Allgather(&rhs[kstart][0][0][0],scount,MPI_DOUBLE,&rhs[0][0][0][0],rcount,MPI_DOUBLE,MPI_COMM_WORLD);
    ...
    errcode = MPI_Allgatherv(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,&rhs[0][0][0][0],rcounts,disps,MPI_DOUBLE,MPI_COMM_WORLD);
